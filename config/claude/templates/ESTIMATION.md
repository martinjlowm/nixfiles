...
throughput data. The baseline was measured over a 6-month window: 177 completed items across 4
GitHub projects in a TypeScript and Rust monorepo. Every number was validated by
recomputing the statistics from per-item raw data.
...
days from sprint start to issue close, so a 2-week sprint is 14 calendar days.
...
- Adding a second senior raises capacity to about 8 items/sprint, not 10.6, because of coordination overhead
...
- Can sustain parallel work alongside seniors at little coordination cost
- Handles 2–3pt items with full autonomy, needs direction on 5pt

#### Junior developer profile
- 1.0–1.5 items/sprint, mostly trivial and small items
- Low cycle times (4–8d) because scope is constrained
- Onboarding overhead inflates initial cycle times by about 50%
- Mostly contributes to 1pt items, and 2pt with pairing or review support
...
- 2 part-time juniors: 1.4 + 1.2 items/sprint
- 1 occasional contributor: about 2 items in 1 sprint
...
- 2 juniors and occasional contributors: 1.0–3.5 items/sprint, over limited sprints
...
1. Points, 1 / 2 / 3 / 5, from the complexity scale in section 1.
2. Code-size bucket, XS / S / M / L / XL, estimated from the implementation
   details. Cross-check it against the point value.
3. Dependencies: the issues that must complete before this one can start.
...
**Scaling is sub-linear.** Coordination overhead means each developer beyond the
second adds only about 80% marginal throughput. Apply a 0.8x discount per
developer after the second:
...
When scheduling, assign 5pt items to senior capacity. Do not allocate juniors
anything above 2pt.

### Step 6: schedule into sprints

1. Sort issues by dependency order, a topological sort.
...
## 4. Worked example: 1 senior and 2 juniors

Team composition:
- 1 senior developer, full-time on the project
- 2 junior developers, full-time on the project
...
Scaling, 3 devs > 2: 1 excess dev at 80%, so discount 1 junior by 20%
Adjusted: 4.5 + 1.2 + (1.2 × 0.8) = 6.66 items/sprint
          8   + 2.5 + (2.5 × 0.8) = 12.5 points/sprint

Overhead, assuming a mixed project at 20%:
...
That creates a **senior bottleneck**. The critical path is governed by the
senior's 8 points/sprint, 6.4 usable after overhead, not the team's 12.5.
...
| 2pt | 4 | 2 | 8 | Senior or mid-level; juniors need pairing |
| 1pt | 5 | 1 | 5 | Anyone |
...
| 4 | 3pt (3) + 3pt (3) | none | none | 6 pts |

This assumes no dependencies between items. Dependencies extend the timeline.
...
**Senior capacity:** <X> points/sprint, the bottleneck limit
...
- **3-point items complete faster than 2-point items**, median 9.5d against
  13.4d. This is real. Experienced engineers self-select into well-scoped complex
  work. Don't assume 3pt always takes longer than 2pt when scheduling.

- **XL code changes don't proportionally increase cycle time** over L changes:
  XL median 11.6d against L median 13.5d. Review and integration overhead
  plateaus. The real cost is lead time, XL median 37d against L median 22d,
  because these items sit in the backlog longer.
...
- **Single-senior projects are throughput-capped.** A dominant contributor
  handling 64% of items caps the project at about 5.3 items/sprint. A second
  senior would raise capacity to about 8 items/sprint, not 10.6.

- **Bug ratio depends on project type.** On UI-heavy projects, 14% of sprint
  capacity goes to bugs. Backend-only projects showed 0%. Adjust the overhead
  reserve accordingly.

- **Junior developers have lower variance, not just lower throughput.** Their
  median cycle times, 4–8d, are shorter than seniors' 9–11d because their scope
  is constrained. But they cannot absorb complex work. If all remaining items are
  3–5pt, juniors sit idle.

- **Negative cycle times exist**, 7 items in the baseline. They happen when items
  close before their assigned sprint starts, usually pre-work or sprint
  reclassification. Exclude them when computing capacity for new estimates.

---

## 7. When to distrust this baseline

- Novel technology or domain. The baseline reflects TypeScript GraphQL, React,
  and Rust work in a monorepo. For unfamiliar stacks or greenfield
  infrastructure, use a 1.5x buffer instead of 1.2x.

- Team composition change. Losing a senior, or onboarding several juniors at
  once, costs 1–2 sprints of reduced throughput.
...
- Baseline age. Generated March 2026. Re-validate against fresh data every 6
  months. Fresh pulls show item counts can shift 10-60% after the fact, from
...
For rapid estimation without the full procedure:
...
**Important:** these assume a uniform complexity distribution. If most points sit
