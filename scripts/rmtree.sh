...
function usage {
  cat <<"EOF"
    ...
    FLAGS:
        ...
        -h: print this help
        -v: verbose mode
        -m: MAIN_BRANCH defaults to `master`. Use this flag to pass a different
            main branch name. You may also set the MAIN_BRANCH environment variable
            to set the main branch name.
EOF
  exit 1
}
...
function rmtree {
  ...
  while [ -n "$1" ]; do
    ...
    if [ ! -d "$repo_root_path/$MAIN_BRANCH" ]; then
      # for legacy reasons, check "master" as a possibility for the main
      # branch
      if [ -d "$repo_root_path/master" ] ; then
        MAIN_BRANCH_PATH="$repo_root_path/master"
      else
    ...
    rm -rf "$1"
    (cd "$MAIN_BRANCH_PATH" && git worktree prune && git branch -D "$branch_name")
