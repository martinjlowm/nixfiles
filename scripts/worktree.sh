...
function _worktree {
  ...
  if command -v codegraph > /dev/null 2>&1; then
    ...
    # Prefer the index the mj fleet publishes per master merge over building
    # one: codegraph-pull owns the whole decision (does this repo publish, are
    # there credentials, is the local copy already at the published commit) and
    # is idempotent, so it needs no `[ ! -d .codegraph ]` guard here. Exit 3
    # means "no published index for this repo" — fall back to a local index,
    # which is still gated on the repo being one we index at all.
    echo "Fetching/refreshing codegraph index in background..."
    {
      codegraph-pull > /dev/null 2>&1
      status=$?
      if [ "$status" = 3 ] && [ -n "$CODEGRAPH_REF" ] && [ ! -d ".codegraph" ]; then
        { codegraph init . && codegraph index .; } > /dev/null 2>&1 ||
          warn "codegraph init failed"
      elif [ "$status" != 0 ] && [ "$status" != 3 ]; then
        warn "codegraph-pull failed — navigate without a graph, or run: codegraph init ."
      fi
    } &
    PIDS+=($!)
  fi
